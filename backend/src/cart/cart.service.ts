...
export class CartService {
  ...
  async addItem(dto: AddToCartDto, userId?: string, guestId?: string) {
    ...
    if (dto.quantity > product.stockQuantity) {
      throw new BadRequestException(`Cannot add ${dto.quantity}. Only ${product.stockQuantity} in stock.`);
    }
    ...
    if (userId) {
      ...
      if (newQty > product.stockQuantity) {
        throw new BadRequestException(`Cannot add more. Exceeds stock of ${product.stockQuantity}.`);
      }
      ...
      let items: GuestCartItem[] = data ? JSON.parse(data) : [];
  ...
  async updateItem(productId: string, dto: UpdateCartItemDto, userId?: string, guestId?: string) {
    ...
    if (dto.quantity > product.stockQuantity) {
      throw new BadRequestException(`Cannot update to ${dto.quantity}. Only ${product.stockQuantity} in stock.`);
    }
